...
function updateIntersectionPlanes!(sc::SubCuboid)
    ...
    # Todo: return all updates from this function, then reflect that updateAndTransfer function
    # as well as Transfer function. Then continue going down this file, updating to the splitting into internal
    # plane, lines and corner return arrays as well as making this reflect in the transfer functions that
    # need to be moved from the SubCuboid file to here. 
    #
    # After this is done, we need to change and create the 
    # updateShell, updateEdge etc. functions so that the calls done in Transfer function works. After that
    # we need to update neighbor functions to use the new edges.
...
function intersectionPlanesTransfer(sc_nb::RemoteNeighbors{SubCuboid}, plane1_update::Vector{Tuple{I,I,LatticeSite}},
    ...
    chan = sc_nb.rnᵣ₋₂
    remotecall_wait(updateShell3!, chan.where, chan, plane4_update, edge24_update, corner245_update)
    # SubCuboid scᵣ₊₁ is affected by updates in plane 1 through its shell[2]
    chan = sc_nb.rnᵣ₊₁
    remotecall_wait(updateShell2!, chan.where, chan, plane1_update, edge15_update, corner135_update)
...
function plane6InternalsTransfer(sc_nb::RemoteNeighbors{SubCuboid}, plane6_update::Vector{Tuple{I,I,LatticeSite}},
    ...
    # TODO: Continue implementing the remaining updateShellEdge functions below and continue to next transfer function. Then see above.
...
function plane6IntersectionLinesTransfer(sc_nb::RemoteNeighbors{SubCuboid}, edge16_update::Vector{Tuple{I,LatticeSite}},
    ...
    chan = sc_nb.rnᵣ₊₂₋₃
    remotecall_wait(updateShellEdge46!, chan.where, chan, corner136_update)
...
# This function sends information to the shells and shell-edges of neighboring sub-cuboids to reflect this.
function plane6IntersectionPointTransfer(sc_nb::RemoteNeighbors{SubCuboid}, edge16_update::Vector{Lattice},
                           edge46_update::Vector{Lattice}, corner_update::Vector{Lattice}) where I<:Int

    # SubCuboid scᵣ₊₁ is affected by updates in edge 16 through its shell[2]
    chan = sc_nb.rnᵣ₊₁
    remotecall_wait(updateShell2!, chan.where, chan, edge16_update, corner_update)
    # SubCuboid scᵣ₋₂ is affected by updates in edge 46 through its shell[3]
    chan = sc_nb.rnᵣ₋₂
    remotecall_wait(updateShell3!, chan.where, chan, edge46_update, corner_update)
    # SubCuboid scᵣ₋₃ is affected by updates in plane 6 through its shell[5]
    chan = sc_nb.rnᵣ₋₃
    remotecall_wait(updateShell5!, chan.where, chan, edge16_update, edge46_update, corner_update)

    # SubCuboid scᵣ₊₁₋₃ is affected by updates in edge 16 through its shell_edge25
    chan = sc_nb.rnᵣ₊₁₋₃
    remotecall_wait(updateShellEdge25!, chan.where, chan, edge16_update, corner_update)
    # SubCuboid scᵣ₊₁₋₂ is affected by updates in corner 146 through its shell_edge23
    chan = sc_nb.rnᵣ₊₁₋₂
    remotecall_wait(updateShellEdge23!, chan.where, chan, corner_update)
