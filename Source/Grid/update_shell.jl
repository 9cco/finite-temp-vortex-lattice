...
function updateShell!(sc::SubCuboid, shell_nr::I, corner_update::Vector{LatticeSite}, i::I, j::I) where I<:Int
    shell = sc.shell[shell_nr]
    for ϕ in corner_update
        shell[i,j] = ϕ
    end
    nothing
end

function updateShell1!Step4(chan::RemoteChannel{Channel{SubCuboid}}, edge26_update::Vector{Tuple{I,LatticeSite}},
                            corner236_update::Vector{LatticeSite}) I<:Int
    sc = take!(chan)
    l₁ = sc.l₁; l₂ = sc.l₂; l₃ = sc.l₃
    shell_nr = 1

    updateShell!(sc, shell_nr, edge26_update, 1)
    if length(corner236_update) > 0
        updateShell!(sc, shell_nr, corner236_update, l₂, 1)
    end
...
function updateShell1!(chan::RemoteChannel{Channel{SubCuboid}}, edge24_update::Vector{Tuple{I,LatticeSite}},
    ...
    if length(corner245_update) > 0
        updateShell!(sc, shell_nr, 1, corner245_update, 1, l₃)
    end
...
function updateShell1!(chan::RemoteChannel{Channel{SubCuboid}}, plane2_update::Vector{Tuple{I,I,LatticeSite}},
    ...
    updateShell!(sc, 1, sc.l₂, edge23_update)
    updateShell!(sc, 1, edge25_update, l₃)
    updateShell!(sc, 1, corner235_update, l₂,l₃)
    put!(chan, sc)
    nothing
end
...
function updateShell2!(chan::RemoteChannel{Channel{SubCuboid}}, plane1_update::Vector{Tuple{I,I,LatticeSite}},
    ...
    if length(corner135_update) > 0
        updateShell!(sc, shell_nr, corner135_update, l₂, l₃)
    end
...
function updateShell3!(chan::RemoteChannel{Channel{SubCuboid}}, plane4_update::Vector{Tuple{I,I,LatticeSite}},
               edge24_update::Vector{Tuple{I,LatticeSite}}, corner245_update::Vector{LatticeSite}) where I<:Int
    ...
    updateShell!(sc, shell_nr, 1, edge24_update)
    if length(corner245_update) > 0
        updateShell!(sc, shell_nr, corner245_update, 1, l₃)
    end
...
function updateShell4Step4!(chan::RemoteChannel{Channel{SubCuboid}}, edge36_update::Vector{Tuple{I,LatticeSite}},
                            corner236_update::Vector{LatticeSite})
    sc = take!(chan)
    l₁ = sc.l₁; l₂ = sc.l₂; l₃ = sc.l₃
    shell_nr = 4
...
function updateShell4!(chan::RemoteChannel{Channel{SubCuboid}}, plane3_update::Vector{Tuple{I,I,LatticeSite}},
    ...
    updateShell!(sc, shell_nr, corner235_update, 1, l₃)
    put!(chan, sc)
    nothing
end

function updateShell5!(chan::RemoteChannel{Channel{SubCuboid}}, plane6_update::Vector{Tuple{I,I,LatticeSite}}, 
                       edge26_update::Vector{Tuple{I,LatticeSite}}, edge36_update::Vector{I,LatticeSite},
                       corner236_update::Vector{LatticeSite})
    sc = take!(chan)
    l₁ = sc.l₁; l₂ = sc.l₂; l₃ = sc.l₃
    shell_nr = 5
    ...
    updateShell!(sc, shell_nr, 1, dge26_update)
    updateShell!(sc, shell_nr, edge36_update, l₂)
    if length(corner236_update) > 0
        updateShell!(sc, shell_nr, corner236_update, 1, l₂)
    end
...
function updateShell6!(chan::RemoteChannel{Channel{SubCuboid}}, edge15_update::Vector{Tuple{I,LatticeSite}},
    ...
    if length(corner245_update) > 0
        updateShell!(sc, shell_nr, corner245_update, 1, 1)
    end
...
function updateShellEdge36!(chan::RemoteChannel{Channel{SubCuboid}}, edge45_update::Vector{Tuple{I,LatticeSite}},
    ...
    if length(corner245_update) > 0
        updateShellEddge!(shell_edge, corner245_update[1], 1)
    end
